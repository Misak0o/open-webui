...
<div class="terminal-command-block bg-gray-900 text-white rounded-lg p-4 my-2">
  <div class="flex items-center justify-between gap-2">
    ...
    <div class="flex gap-1">
      {#if isShellLang(command)}
        <button class="copy-btn px-2 py-1 rounded bg-gray-700 hover:bg-gray-600" on:click={copyCommand}>
          {#if copied}✅ Copied{:else}Copy{/if}
        </button>
        {#if canRun}
          <button class="run-btn px-2 py-1 rounded bg-blue-700 hover:bg-blue-600" on:click={runCommand} disabled={running}>
  ...
  {#if isShellLang(command)}
    ...
    {:else if output}
      <div class="mt-2 text-xs text-green-300">Sortie : <pre>{output}</pre></div>
    {/if}
    {#if error}
      <div class="mt-2 text-xs text-red-400">Erreur : <pre>{error}</pre></div>
    {/if}
  {/if}
</div>
